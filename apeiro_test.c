...
/*  TA1CCTL0 = CCIE;                          // CCR0 interrupt enabled
    ...
    */
    ...
    while(1)
    {
        //everything will happen within the while loop
        P1OUT ^= 0x01;                            // Toggle P1.0 which is LED. one blink is one reading

        //Telecommand - checks everytime it enters while
        //Counter reset : P8.5 input
        ...
        while(!(UCA2IFG&UCRXIFG))
        {
           counter++;
           __delay_cycles(20);
           if(counter == 10000) //wait for atleast 2s = 2000ms
               goto IMU;
        }                                   // Byte is received
